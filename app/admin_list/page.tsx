...
export default function AdminListPage() {
  ...
  // Filter and sort admins dynamically
          ...
          <>
            ...
            <div className="mobile-cards" style={{ flexDirection: 'column', gap: '0.75rem', padding: '1rem' }}>
              {filteredAdmins.map((admin, index) => {
                ...
                const isTop2 = sortBy === 'score' && index === 1
                const isTop3 = sortBy === 'score' && index === 2
                ...
                return (
                  <div key={admin.id} style={{ 
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                        <span style={{ 
                          fontWeight: 700, 
                          fontSize: '0.9rem', 
                          color: isTop1 ? '#d97706' : isTop2 ? '#475569' : isTop3 ? '#b45309' : '#64748b', 
                          width: '24px' 
                        }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #e2e8f0', paddingTop: '0.6rem', fontSize: '0.85rem' }}>
                      <span style={{ padding: '0.2rem 0.5rem', borderRadius: '4px', fontSize: '0.72rem', background: '#eff6ff', color: '#1d4ed8', fontWeight: 600 }}>{admin.role || 'Admin'}</span>
            ...
            <table className="desktop-table" style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.9rem' }}>
              <thead>
                <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                  ...
                  <th style={{ padding: '0.75rem 1rem', textAlign: 'right', fontWeight: 600, color: '#475569' }}>Productivity Score</th>
                </tr>
              </thead>
              <tbody>
                {filteredAdmins.map((admin, index) => {
                  ...
                  return (
                    <tr key={admin.id} style={{ borderBottom: '1px solid #f1f5f9', background: isTop1 ? '#fffbeb' : 'transparent' }}>
                      <td style={{ padding: '0.75rem 1rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                          <span style={{ 
                            fontWeight: 700, 
                            width: '24px', 
                            color: isTop1 ? '#d97706' : isTop2 ? '#64748b' : isTop3 ? '#b45309' : '#64748b' 
                          }}>
                            {sortBy === 'score' ? `#${index + 1}` : '•'}
                          </span>
                          {admin.avatar_url ? (
                      ...
                      <td style={{ padding: '0.75rem 1rem', textAlign: 'right', fontWeight: 700, color: '#10b981', verticalAlign: 'middle' }}>
                        {admin.score || 0} pts
                      </td>
                    </tr>
                  )
