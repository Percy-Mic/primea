...
export default function LoggedActionPage() {
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '2rem auto', padding: '1rem', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      {activeTab === 'chat' ? (
        <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '12px', display: 'flex', flexDirection: 'column', height: '550px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.02)', overflow: 'hidden' }}>
          ...
          <div style={{ flex: 1, padding: '1rem', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0.75rem', background: '#f8fafc', minWidth: 0 }}>
            {messages.length === 0 ? (
              ...
              messages.map((msg) => {
                ...
                return (
                  <div key={msg.id} style={{ display: 'flex', justifyContent: isMe ? 'flex-end' : 'flex-start', alignItems: 'flex-end', gap: '0.5rem', width: '100%', minWidth: 0 }}>
                    ...
                    <div style={{ maxWidth: '85%', width: '100%', minWidth: 0, background: isMe ? '#2563eb' : '#fff', color: isMe ? '#fff' : '#1e293b', padding: '0.65rem 0.9rem', borderRadius: isMe ? '18px 18px 4px 18px' : '18px 18px 18px 4px', boxShadow: '0 1px 2px rgba(0,0,0,0.05)', border: isMe ? 'none' : '1px@ solid #e2e8f0', overflow: 'hidden' }}>
                      ...
                      {msg.audio_url && (
                        <div style={{ marginTop: msg.message ? '0.5rem' : 0, display: 'flex', alignItems: 'center', gap: '0.75rem', background: isMe ? 'rgba(0,0,0,0.1)' : '#f1f5f9', padding: '0.4rem 0.75rem', borderRadius: '20px', width: '100%', boxSizing: 'border-box' }}>
                          <button
                            type="button"
          ...
          <form onSubmit={handleSendMessage} style={{ padding: '0.75rem 1rem', background: '#fff', borderTop: '1px solid #e2e8f0', display: 'flex', alignItems: 'flex-end', gap: '0.5rem', width: '100%', boxSizing: 'border-box' }}>
            ...
            {isRecording ? (
              ...
              <div style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#f1f5f9', borderRadius: '24px', padding: '0.3rem 0.75rem', boxSizing: 'border-box' }}>
                <audio controls src={audioUrlPreview} style={{ height: '30px', width: '100%', maxWidth: '220px' }} />
                <button type="button" onClick={cancelRecording} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer', fontSize: '0.8rem', fontWeight: 600, flexShrink: 0, marginLeft: '0.5rem' }}>Discard</button>
              </div>
            ) : (
              <div style={{ flex: 1, minWidth: 0, display: 'flex', background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '20px', padding: '0.65rem 1rem', boxSizing: 'border-box' }}>
                <textarea
                  ref={textareaRef}
                  rows={1}
